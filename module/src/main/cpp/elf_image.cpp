...
ElfImage::ElfImage(std::string_view lib_name) : path_(lib_name) {
    ...
    if (fd < 0) {
        LOGE("Failed to open ELF file: %s", path_.c_str());
        return;
    }

    struct stat file_info;
    if (fstat(fd, &file_info) < 0) {
        LOGE("fstat failed for %s", path_.c_str());
        close(fd);
        return;
    }
    ...
    if (file_map_ == MAP_FAILED) {
        LOGE("mmap failed for %s", path_.c_str());
        file_map_ = nullptr;
        return;
    }
    ...
    for (int i = 0; i < header->e_shnum; ++i) {
        ...
        switch (section_h->sh_type) {
            ...
            case SHT_PROGBITS:
                ...
                if (!bias_calculated_ && section_h->sh_flags & SHF_ALLOC &&
                    section_h->sh_addr > 0) {
...
bool ElfImage::decompressGnuDebugData() {
    ...
    LOGD("Found .gnu_debugdata section in %s (%zu bytes). Decompressing...", path_.c_str(),
         static_cast<size_t>(debugdata_size));
    ...
    while (true) {
        ...
        if (ret == XZ_STREAM_END) {
            ...
            xz_dec_end(dec);
            LOGD("Successfully decompressed .gnu_debugdata (%zu bytes)", elf_debugdata_.size());
            return true;
        }
        if (ret != XZ_OK) {
            LOGE("XZ decompression failed with code %d", (int) ret);
            xz_dec_end(dec);
            return false;
        }
...
ElfW(Addr)
ElfImage::getSymbOffset(std::string_view
                        name,
                        uint32_t gnu_hash,
                        uint32_t
                        elf_hash) const {
...
ElfW(Addr)
ElfImage::gnuLookup(std::string_view
                    name,
                    uint32_t hash
) const {
    ...
    do {
        ElfW(Sym)
                *
                sym = dynsym_start_ + sym_idx;
        if (((gnu_chain_[sym_idx - gnu_symndx_] ^ hash) >> 1) == 0) {
            if (
                    std::string_view(strtab_start_
                                     + sym->st_name) == name) {
                return sym->
                        st_value;
            }
...
ElfW(Addr)
ElfImage::elfLookup(std::string_view
                    name,
                    uint32_t hash
) const {
...
void ElfImage::ensureLinearMapInitialized() const {
    ...
    for (ElfW(Off) i = 0; i < symtab_count_;
         ++i) {
...
ElfW(Addr)
ElfImage::linearLookup(std::string_view
                       name) const {
...
ElfW(Addr)
ElfImage::prefixLookupFirst(std::string_view
                            prefix) const {
    ensureLinearMapInitialized();
...
bool ElfImage::findModuleBase() {
    ...
    while (fgets(line_buffer, sizeof(line_buffer), maps)) {
        if (strstr(line_buffer, path_.c_str())) {
            ...
            char p[5] = {0};
            int items_parsed =
                    sscanf(line_buffer, "%llx-%*x %4s %*x %*s %*d %255s", &temp_start, p,
                           path_buffer);
            ...
            if (items_parsed >= 2) {
                ...
                LOGD("Found module entry: %s", line_buffer);
            }
        }
    }
    fclose(maps);
    ...
    if (filtered_list.empty()) {
        LOGE("Could not find any mappings for %s", path_.c_str());
        return false;
    }
    ...
    LOGD("Found base for %s at %zu", path_.c_str(), found_block->start_addr);
    return true;
}
